<div class="min-h-screen bg-gray-100 dark:bg-gray-900 flex items-center justify-center py-4 sm:py-10 px-4">
    <div class="w-full max-w-4xl px-4">
        @if($template && $template->imagen_plantilla && $template->variables_config)
            <!-- Carnet con diseño personalizado -->
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden border border-gray-200 dark:border-gray-700 p-4 sm:p-6">
                <div class="relative w-full flex justify-center">
                    @if($previewImageUrl)
                        <!-- Mostrar imagen generada por backend (igual al PDF) -->
                        <img src="{{ $previewImageUrl }}" 
                             alt="Vista previa del carnet" 
                             class="w-full max-w-full h-auto border border-gray-300 rounded-lg object-contain"
                             style="max-height: 80vh; width: auto;"
                             id="carnet-preview-image">
                    @else
                        <!-- Fallback: Canvas si no se pudo generar la imagen -->
                        <canvas id="carnet-canvas" class="w-full max-w-full h-auto border border-gray-300 rounded-lg" style="max-height: 80vh; width: auto;"></canvas>
                    @endif
                </div>
        ...
        @else
            ...
            <div class="w-full max-w-md mx-auto bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden border border-gray-200 dark:border-gray-700">
