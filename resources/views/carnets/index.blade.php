    ...
    <div class="max-w-7xl mx-auto py-8 px-6">
        ...
        <div class="mb-6 bg-blue-50 dark:bg-blue-900 border border-blue-200 dark:border-blue-700 rounded-lg p-4">
            <div class="flex items-start space-x-3">
                <svg class="w-5 h-5 text-blue-600 dark:text-blue-300 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                </svg>
                <div class="flex-1">
                    <p class="text-sm font-semibold text-gray-800 dark:text-gray-100 mb-2">Tamaños Recomendados para Plantilla de Carnet:</p>
                    ...
                    <p class="text-xs text-gray-700 dark:text-gray-300 mt-2">Formatos aceptados: JPG, PNG, GIF. Máximo 5MB</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow border border-gray-200 dark:border-gray-700 rounded-lg p-6">
            <div class="flex justify-between items-center mb-6">
                <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-100">Plantilla de Carnet</h3>
            ...
            @if($template && $template->imagen_plantilla)
                <div class="mb-4">
                    <p class="text-sm text-gray-700 dark:text-gray-300 mb-2">Vista previa de la plantilla:</p>
                    <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 inline-block">
                        @php
            ...
            @if($template && $template->variables_config)
                <div class="mt-6">
                    <h4 class="text-md font-semibold text-gray-800 dark:text-gray-100 mb-3">Variables configuradas:</h4>
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                        @foreach($template->variables_config as $var => $config)
                            @if(isset($config['activo']) && $config['activo'])
                                <div class="flex items-center space-x-2 bg-gray-50 dark:bg-gray-700 px-3 py-2 rounded">
                                    <svg class="w-4 h-4 text-green-500" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
                                    </svg>
                                    <span class="text-sm text-gray-700 dark:text-gray-300">{{ $var }}</span>
                                </div>
                            @endif
                        @endforeach
                    </div>
        ...
        <!-- Botón para exportar carnets -->
        <div class="mt-6 bg-white dark:bg-gray-800 shadow border border-gray-200 dark:border-gray-700 rounded-lg p-6">
