...
<x-app-layout>
    ...
    <div class="max-w-5xl mx-auto py-8 px-6">
        <div class="{{ $bgCard }} shadow border {{ $borderCard }} rounded-lg p-6">
            <form method="POST" action="{{ route('vehiculos.update', $vehiculo) }}" enctype="multipart/form-data" class="space-y-4">
                @csrf
                @method('PUT')

                @include('vehiculos.form', ['vehiculo' => $vehiculo, 'theme' => $theme, 'isDark' => $isDark])
